...
const sendMailResetPasswordMail = async (receipantName: string, receipantMailId: string, resetLink: string) => {
  if (!receipantMailId) {
    return false;
  }
  try {
...
const sendMailGotFeedback = async (receipantName: string, receipantMailId: string, subject: string, message: string) => {
  if (!receipantMailId) {
    return false;
  }
  try {
...
const sendMailFeedbackResponse = async (receipantName: string, receipantMailId: string) => {
  if (!receipantMailId) {
    return false;
  }
  try {
...
export {sendMailFeedbackResponse, sendMailGotFeedback, sendMailResetPasswordMail};
